            ...
            <div>
                <div class="filterboxx" style="margin-top: 16px">
                    ...
                    <div class="filterbox" :class="{ 'expand': filterShow }">
                        ...
                        <div>
                            <button class="filterbtn" @click="handleFilter"><i class="fi fi-rr-filter"></i> Filter
                                Data</button>
                        </div>
                    </div>
                </div>
                <div class="boxinfo">
                    <p class="success">Total unit show : <strong v-if="listEq.length"> {{ filterpage ? filtercount :
                        listEq.length }} </strong> from {{ totalData ? totalData : listEq.length }} units.</p>
                    <label class="labs">
                        <i class="fi fi-rr-search"></i>
                        <input type="text" class='search' placeholder="Type unit number ... " @keyup="handlesearch"
                            v-model="searchparam">
                    </label>
                </div>
            </div>
            <div class="infopage">
                <div class="togle">
                    <div style="display: flex; align-items: center; gap: 16px"
                        v-if="exChecked != '' || dtChecked != '' || hdChecked != '' && listEq.length">
                    ...
                    <p v-if="isAutoUpdate">Displaying page <strong>{{ activepage }}</strong> from total
                        <strong>{{ totalpage }}</strong> pages.
                    </p>
                </div>
            </div>
            <div class="outbox">
...
export default {
    ...
    methods: {
        loadData(data) {
            ...
            this.listEq = data
            // count unit
            axios.get(`https://api.ppa-mhu.net/ppa-employee-api/api/cico/listUnit?units[]=${this.hdChecked ? 'HD' : ''}&units[]=${this.exChecked ? 'EX' : ''}&units[]=${this.dtChecked ? 'DT' : ''}&page=${this.filterpage ? this.filterpage : 1}&count=${filtercount}`)
                .then(res => this.countUnit(res.data.data.length))
        },
        handlesearch() {
            let filtered = this.storageData.filter(it => it.CN.toLowerCase().includes(this.searchparam.toLowerCase()))
